...
### 단위 테스트 항목
- InputValidator
  - [x] validateTotalPurchaseAmountFormat() 테스트
  - [x] validateLottoWinningNumbersFormat() 테스트
  - [x] validateLottoBonusNumberFormat() 테스트
- TotalPurchaseAmount
  - [x] 천원 단위가 아닐 경우 예외발생
  - [x] 천원 이상이 아닐 경우 예외발생
  - [x] 정상 생성
...
- LottoResult
  - [x] 등수별 초기값 0인지 테스트
  - [x] 등수를 추가하면 등수의 카운트가 증가하는지 테스트
  - [x] getResult() 반환값은 복사본인지 확인
  - [x] getTotalPrize() 정상 반환 확인
  - [x] 수익률 계산로직 정상작동 테스트
- LottoGenerator
  - [x] 금액만큼 로또 생성되는지 확인
- Lotto
  - [x] 로또번호 정상 생성 확인
  - [x] 로또번호 6개인지 확인
  - [x] 로또번호 1~45를 벗어나면 예외 발생
  - [x] 로또번호 중복시 예외 발생
- Lottos
  - [x] 로또를 추가했을때 정상적으로 조회되는지 확인
